...
class Gate_Manager:
    ...
    def __init__(self, gates_file, backuup_dir) -> None:
        self.gates_file = gates_file
        self.load_gates()
        self.backup_dir = backup_dir

    def select_gates_file(self):
        files = os.listdir(backup_dir).reverse()
    ...
    def load_gates(self, gates_file = gates_file): 
        ...
        self.gates = gates
        return(gates)
        #print(f'These are your gates {gates}')
    ...
    def set_gate_name(self, gate_key):
        ...
        for g in self.gates:
            ...
            print(f"Changing {my_gate.name} to {new_name}")
    ...
    def set_gate_pin(self, gate_key):
        #my_gate = self.gates[gate_key]
        # get requested pin
        requested_pin = int(q.text("What pin is the gate on? (0 - 16)", style = style).ask())
        if 0 <= requested_pin <= 16:
            if len(self.gates) != 0:
                for g in self.gates:
                    s_gate = self.gates[g]
    ...
    def delete_gate(self, gate_key):
        my_gate= self.gates[gate_key]
        confirm = q.confirm(f"Are you sure you want to remove {my_gate.name} at location {my_gate.location}?", style = style).ask()
        if confirm:
    ...
    def add_new_gate(self):
        ...
        if self.set_gate_pin('new') != True:
            self.delete_gate('new')
            return False
        if self.set_location('new') != True:
            self.delete_gate('new')
            return False
        if self.set_min('new') != True:
            self.delete_gate('new')
            return False
        if self.set_max('new') != True:
            self.delete_gate('new')
            return False
        if self.set_info('new') != True:
            self.delete_gate('new')
            return False
        if self.set_gate_name('new') != True:
            self.delete_gate('new')
            return False
        return True
